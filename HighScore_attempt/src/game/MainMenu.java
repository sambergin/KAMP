...
public class MainMenu extends MouseAdapter{
	...
	public MainMenu(Game game, Handler handler, LoadStrings temp, HighScores tempM) {
		this.temp = temp;
		this.tempM = tempM;
		this.game = game;
	...
	public void render(Graphics g) throws IOException {
		...
		if(game.gameState == STATE.MouseHSMenu || game.gameState == STATE.TypeHSMenu) {
			...
			g.drawRect(Game.WIDTH/2-100, 500, 200, 64);
			for(int i = 0; i < game.mouseHS.size(); i++) {
				g.drawString(game.mouseHS.get(i), Game.WIDTH/2-50, 150+i*25);
			}
			g.drawString("Return", Game.WIDTH/2-50, 540);			
		...
		if(game.gameState == STATE.MouseEndMenu) {
			...
			HighScores.newMHS(game.hud.score);
			//Main Menu Button
			g.setColor(Color.MAGENTA);
			g.fillRect(Game.WIDTH/2-100, 500, 200, 64);
			g.setColor(Color.ORANGE);
